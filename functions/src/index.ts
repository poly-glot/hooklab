...
// ── 1. Seed guest data ─────────────────────────────────────────────
export const seedGuestData = onCall(
...
return {
  ...
  body: JSON.stringify({ ok: true, timestamp: Date.now() })
      ...
      for (let i = 0; i < Math.min(sample.requestCount, 3); i++) {
        ...
        batch.set(execRef, {
          ...
          timestamp: Timestamp.fromMillis(
            Date.now() - i * 60000 * (Math.random() * 10 + 1)
          ),
        });
      }
    }
    ...
    batch.set(
      userRef,
      { seeded: true, endpointCount: sampleEndpoints.length },
      { merge: true }
    );
...
export const recordExecution = onCall(
  ...
  async (request) => {
    ...
    if (!endpointId || !execution) {
      throw new HttpsError("invalid-argument", "endpointId and execution required");
    }
    ...
    if (!endpointSnap.exists || endpointSnap.data()?.userId !== uid) {
      throw new HttpsError("permission-denied", "Endpoint not found");
    }
    ...
    await firestore.runTransaction(async (tx) => {
      ...
      tx.set(
        userRef,
        { quotas: { usedExecutionsToday: FieldValue.increment(1) } },
        { merge: true }
      );
    });
...
export const clearExecutions = onCall(
  { enforceAppCheck: !isEmulator, region },
  async (request) => {
    const uid = request.auth?.uid;
    if (!uid) {
    ...
    if (!endpointId) {
      throw new HttpsError("invalid-argument", "endpointId is required");
    }
...
export const cleanupOldExecutions = onSchedule(
  ...
  async () => {
    ...
    console.log(
      `Starting cleanup for executions older than ${cutoffDate.toISOString()}`
    );
    ...
    console.log(
      `Cleanup completed: ${totalDeleted} old executions deleted, quotas reset`
    );
  }
);
...
export const aggregateAnalytics = onSchedule(
  ...
  async () => {
    ...
    console.log(
      `Analytics aggregated for ${todayStr}: ${executions.size} executions`
    );
  }
);
